...
async function run() {
  try {
    ...
    const blogCollection = client.db('Volunteers').collection('blogs');
    ...
    app.get('/MyRequestPost', logger, verifyToken, async (req, res) => {
    ...
    app.get('/manageMyPost', logger, verifyToken, async (req, res) => {
      const tokenEmail = req.user.email;
      if (tokenEmail !== req.query.email) {
        return res.status(403).send({ message: 'forbidden access' })
      }
    ...
    // my requested volunteer post cancelling api route
    app.get('/MyRequestPost/:email', logger, verifyToken, async (req, res) => {
      const tokenEmail = req.user.email;
